"""Convert OCLC_FLIN_Cleaned.xlsx into flin-policies.json.

Same shape as build_lvis_policies / build_film_policies. The source
spreadsheet has the standard 10-column layout (Institution, Library/Branch,
Symbols, Supplier, Copies Days, Loans Days, Copies Fees, Loans Fees,
Location, Notes); we drop the constant Supplier column and the empty
Notes column.

Run: python3 scripts/build_flin_policies.py
"""
...
SRC = ROOT / "OCLC_FLIN_Cleaned.xlsx"
DST = ROOT / "flin-policies.json"
...
def main():
    ...
    out = {
        "_meta": {
            "description": (
                "FLIN (Florida Library Information Network) library data used "
                "by Lender Finder. Each entry's `policyUrl` is that "
                "institution's OCLC ILL policies page — use it for 'View OCLC "
                "policies' / 'Load policies from OCLC' instead of running a "
                "symbol search."
            ),
            "source": SRC.name,
            "group": "FLIN",
            ...
            "fields": {
                ...
                "type": "Best-guess library type (Academic/Archives/Consortium/Corporate/Government/Law/Medical/Museum/Music/Public/School/Special/Other) inferred from name",
                "group": "Always 'FLIN' for this dataset",
            },
        },
        "libraries": libraries,
    }
