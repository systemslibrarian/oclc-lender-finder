...
def build_geocoder():
    """Return a (city_lower, state_code) -> (lat, lng) lookup and a state-only fallback,
    using the pgeocode US postal-code dataset. Returns (None, None) if pgeocode is missing.
    """
    if pgeocode is None:
        return None, None
